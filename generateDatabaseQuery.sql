...
BEGIN
    ...
    THEN
        BEGIN
            INSERT INTO Alerts (userAcc, message)
            SELECT userAcc, "ALERT - A product in your AlertsList has been listed." FROM AlertsList
            WHERE category = NEW.category AND brand = NEW.brand
              AND damageCondition = NEW.damageCondition AND color = NEW.color;
        END;
    END IF;
...
BEGIN
    ...
    THEN
        BEGIN
            ...
            INSERT INTO Alerts (userAcc, message)
                (SELECT b.buyerAccount, "ALERT - Congratulations, you won an auction!" FROM Bid b
                 WHERE b.productId = NEW.productId);
...
BEGIN
    ...
    THEN
        BEGIN
            DELETE FROM AutoBidder WHERE userAcc = NEW.username;
        END;
    END IF;
END; !!!
DELIMITER ;

# Safety check to prevent deletion of the Administrator UserAccount.
...
CREATE EVENT PastDue ON SCHEDULE EVERY 1 HOUR STARTS NOW() DO
    BEGIN
        UPDATE Product SET isSold = true WHERE NOW() > endDate AND price >= reservePrice;
